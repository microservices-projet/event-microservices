...
/**
 * Tries Keycloak JWT first, then local HS256 (user-service / gateway).
 * Keycloak decoder is created lazily so the app can start before Keycloak is up; creation is retried until it succeeds.
 */
@Configuration
    ...
    private static final class LazyCompositeJwtDecoder implements JwtDecoder {
        ...
        public Jwt decode(String token) throws JwtException {
            ...
            try {
                ...
                // Issuer metadata not reachable yet (Keycloak down or realm not imported).
            }
            if (kc != null) {
                try {
